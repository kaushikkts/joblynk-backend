...
import {getAWSSecretKey} from "../helper/get-database-password";
const secretManager = new AWS.SecretsManager({region: 'us-east-1'});
const key: any = 'awsSecretKey';
export const verifyEmail = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        //         Subject: {
        //             Charset: 'UTF-8',
        //             Data: 'http://localhost:4200/sign-up'
        //         }
        //     },
        //     Source: 'kkarandikar08@gmail.com', /* required */
        //
        // };
        const secretKey = await getAWSSecretKey();
        AWS.config.update({
            region: 'us-east-1',
            apiVersion: "latest"
        })
        let verifyEmail = await new AWS.SES({credentials: {
            accessKeyId: 'AKIAVA5O6P5STZGQMGNW',
                secretAccessKey: ''
            }}).verifyEmailIdentity({EmailAddress: 'admin@ktechsolutions.org'}).promise();
